...
export default function ArticleSidebar({ article }: { article: Article }) {
  const tocItems = article.body.filter((b) => b.type === 'h2') as Array<{
    type: 'h2'
    text: string
    id: string
  }>
    ...
    <aside className="w-full lg:w-[196px] lg:sticky lg:top-[88px] lg:self-start">
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-3">
          <span
            className="w-[18px] h-[18px] rounded-full bg-ink flex items-center justify-center"
            aria-hidden="true"
          >
            <svg
              width="10"
        ...
        <div className="flex flex-col gap-6">
          <h2 className="font-display font-bold text-ink text-[20px] leading-none">
            In this article
          </h2>
          <nav className="flex flex-col gap-6">
            {tocItems.map((item) => {
              ...
              const linkClass =
                'font-body font-semibold text-[14px] leading-[1.1] transition-colors ' +
